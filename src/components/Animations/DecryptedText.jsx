  ...
  useEffect(() => {
    ...
    intervalRef.current = setInterval(() => {
      setRevealedIndices(prevRevealed => {
        if (sequential || revealMode === 'word') {
          if (direction === 'forward') {
            if (revealMode === 'word') {
              const nextWordIndices = wordIndices.find(word => !word.every(idx => prevRevealed.has(idx)));
              if (nextWordIndices) {
                const newRevealed = new Set(prevRevealed);
                nextWordIndices.forEach(idx => newRevealed.add(idx));
                const lastIdx = nextWordIndices[nextWordIndices.length - 1];
                if (text[lastIdx + 1] === ' ') newRevealed.add(lastIdx + 1);
